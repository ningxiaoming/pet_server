...
public class ImageService {
    ...
    public ResultInfo uploadImage(MultipartFile image){
        ...
        String filePath = "test/"+(idWorker.nextId())+"."+ StringUtils.substringAfterLast(image.getOriginalFilename(), ".");
        // 上传到阿里云
        try {
    ...
    public ResultInfo uploadBanner(MultipartFile image){
        ...
        IdWorker idWorker = new IdWorker(1,1,1);
        String filePath = "pet_service/public/banner/"+(idWorker.nextId())+"."+ StringUtils.substringAfterLast(image.getOriginalFilename(), ".");
        // 上传到阿里云
        try {
            ossClient.putObject(aliyunConfig.getBucketName(), filePath, new
                    ByteArrayInputStream(image.getBytes()));
